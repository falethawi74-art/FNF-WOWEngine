...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		EngineWatermark.setFormat(Paths.font("vcr.ttf"), 16, 0xFFFF0000, RIGHT, OUTLINE,FlxColor.BLACK);
		...
		{
			case 'WOW': EngineWatermark.text = SONG.song + " " + CoolUtil.difficultyString() + " | WOW Engine v" + MainMenuState.WOWEngineVersion;

			default:
		}

		scoreTxt = new FlxText(0, healthBarBG.y + 36, FlxG.width, "", 20);
		scoreTxt.setFormat(Paths.font("vcr.ttf"), 20, 0xFFFF0000, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("vcr.ttf"), 32, 0xFFFF0000, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override public function update(elapsed:Float)
	{
		...
		var mult:Float = FlxMath.lerp(1, iconP1.scale.x, CoolUtil.boundTo(1 - (elapsed * 32), 0, 1));
		iconP1.scale.set(mult, mult);
		iconP1.updateHitbox();

		var mult:Float = FlxMath.lerp(1, iconP2.scale.x, CoolUtil.boundTo(1 - (elapsed * 32), 0, 1));
